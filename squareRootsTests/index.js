...
function squareRootsTests() {
    ...
    {
        console.log('тест 2,5,0 -> два корня 0, -2,5');
        const roots = squareRoots(2,5,0);
        console.log(((roots.length == 2) && (roots[0] == 0) && (roots[1] == -2,5))
            ? 'пройден' : 'НЕ ПРОЙДЕН!')
    }

    {
        console.log('тест 2,0,0 -> один корень 0');
        const roots = squareRoots(2,0,0);
        console.log(((roots.length == 1) && (roots[0] == 0))
            ? 'пройден' : 'НЕ ПРОЙДЕН!')
    }

    {
        console.log('тест 0,0,-10 -> корней нет');
        const roots = squareRoots(0,0,-10);
        console.log((!roots.length)
            ? 'пройден' : 'НЕ ПРОЙДЕН!')
    }

    {
        console.log('тест 0,0,0 -> бесконечность решений');
        const roots = squareRoots(0,0,0);
        console.log((roots[0] === Infinity)
            ? 'пройден' : 'НЕ ПРОЙДЕН!')
    }
}
